...
/*
 * 	该类与LdapPasswordUtil结构相同，不同点在于 曹瑞 2015-5-20 将接口的地址拆分为两个，认证不变，修改密码的接口由10.135.7.142改为10.135.7.96，以解决手机端密码修改后，无法登录pc端的问题。
	上述问题原因 10.135.7.142为portal的认证树，10.135.7.96为身份树，只修改认证数的话，身份树的密码有效期没有变化，对于其他系统来说密码仍是过期的。修改了身份树后，portal会同步其他系统中的有效期
*/
public class LdapPasswordUtilForUpdatePass {
	private static final Logger LOGGER = LoggerFactory.getLogger(LdapPasswordUtilForUpdatePass.class);
	private static int ldapPort = LDAPConnection.DEFAULT_PORT;
	private static int ldapVersion = LDAPConnection.LDAP_V3;
	private static String sysLdapHost = Env.getProperty(Env.LDAP_HOST_FOR_UPDATEPASS);// 测试 10.135.7.92     生产 10.135.7.96   10.135.7.142
	private static String sysDN = Env.getProperty(Env.LDAP_SYSUSER_DN_FOR_UPDATEPASS);//"cn=690YD,o=services";// "cn=690YDUser,ou=user,o=services";[3] "cn=testuser,ou=user,o=services";//
	private static String sysPassword = Env.getProperty(Env.LDAP_SYSUSER_PASSWORD_FOR_UPDATEPASS); // 测试 "690YD" 生产 haier,123  "690YDUser@20141128";//[3]"testuser";//
...
//		}
	...
	public static void main(String[] args) {
	   LdapPasswordUtilForUpdatePass ldapPasswordUtil = new LdapPasswordUtilForUpdatePass();
